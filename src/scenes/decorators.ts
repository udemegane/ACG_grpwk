 ...
 */
export function visibleInInspector(
  type: VisiblityPropertyType,
  name?: string,
  defaultValue?: any
): any {
 ...
 */
export function onPointerEvent(
  type: PointerEventTypes,
  onlyWhenMeshPicked = true
): any {
  return (target: any, propertyKey: string | symbol) => {
    if (typeof target[propertyKey] !== 'function') {
      throw new Error(
        `Decorated propery "${propertyKey.toString()}" in class "${
